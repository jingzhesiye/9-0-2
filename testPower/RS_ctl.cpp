...
{
    QStringList strList;
    ui->RS_charger_TblWidget->item(0,1)->setText(QString::number(pRSTYPE_Temp->VOLTAGEOUT,'d',1));
    ui->RS_charger_TblWidget->item(1,1)->setText(QString::number(pRSTYPE_Temp->CURRENCEOUT,'d',1));//实时电流输出.A

    ui->BMS_RD_TblWidget->item(2,1)->setText(QString::number(pRSTYPE_Temp->VOLTAGENEED,'d',1));//充电电压需求.v
    ui->BMS_RD_TblWidget->item(3,1)->setText(QString::number(pRSTYPE_Temp->CURRENCENEED,'d',1));//充电电流需求.A
    ...
    {
       ui->BMS_chargeMode_Label->setText(QString::fromUtf8("无状态"));
    }
    else if(pRSTYPE_Temp->CHARGEMODE==1)
    {
        ui->BMS_chargeMode_Label->setText(QString::fromUtf8("恒压"));
    }
    else if(pRSTYPE_Temp->CHARGEMODE==2)
    {
